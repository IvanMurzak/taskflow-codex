...
directory and each opening its own PR. Optional execution review uses a separate
reviewer agent.
...
```text
$taskflow:taskflow-plan device-pairing
...
```text
$taskflow:taskflow-review device-pairing
```

Three adversarial review lenses try to prove the plan wrong: against your code,
...
```text
$taskflow:taskflow-tasks device-pairing
```

The plan becomes numbered, immutable task specs — grouped into conflict domains
designed to avoid overlapping files and resources — plus a status board in
`ROADMAP.md`.

### 4. Execute

```text
$taskflow:taskflow-execute device-pairing
```
...
what a worker claims, one task PR at a time per slot, board updated after each
one. By default, a PR merges only after DoD, required CI, and owner gates are
green; execution review is required only when `--review` is enabled.

Want it faster, and reviewed as it goes? Native git worktrees isolate each
concurrent worker; the Pipeline CLI is optional:

```text
$taskflow:taskflow-execute device-pairing --parallel=4 --review=high
```
...
half-finished run can never lie to you about where it got to. When integration
is active, a second repo-level table tracks every final integration PR
separately from task rows.
...
| `taskflow-review` | Applies three adversarial review lenses — repository truth, external conformance, internal consistency — then applies the confirmed non-product corrections in one batch. Product questions come back to you. | corrections in place |
...
Any user or agent may invoke any stage. Plan, review, and task generation inherit
the session model; execute applies each ROADMAP row's model-routing hint when the
current Codex spawn interface supports it.
...
lockstep with `skills/taskflow-execute/SKILL.md`, which wins if the two ever
disagree.

| Flag | Values | Default | Notes |
|---|---|---|---|
| `<slug>` (positional) | a taskflow folder under `.taskflow/` | the only folder there | ambiguity is resolved with you, never guessed |
| `--scope=` | `all` · `wave:N` · `group:B` · comma-separated id list | `all` | explicit scope only; ambiguous free text is not interpreted as task selection |
...
| `--integration-branch=` | `auto` · `off` · branch name | `auto` | outside pipeline tier, creates one integration ref per repository and one final PR; incompatible with `--merge=never` unless set to `off`; pipeline tier resolves it to `off` |
...
number of ready groups, the fixed ceiling of 8, and **the host's current
concurrency slots minus the scheduler**. Codex supplies that cap to the
orchestrator; Taskflow does not assume a global default.
...
| `pipeline` | Explicit `--engine=pipeline --pipeline=<name>` plus the Codex Pipeline plugin | Each task is delegated through `$pipeline:run` in Codex-native `manager` mode; that pipeline owns implement → review → PR → CI → merge → sync. Taskflow resolves integration off, and `--merge` does not apply here |
...
On the measured `codex-cli 0.147.0`, two subagents spawned concurrently reported the
...
current Codex session and requires its Codex-native `manager` mode. It stops on
any other declared runner and never launches an external agent process.
...
The chosen worktree root must already be writable under the current Codex
sandbox. When a sibling or temporary root is outside the authorized workspace,
restart Codex with that directory added (for example, `--add-dir <root>`) or
choose an already-authorized root. Taskflow never weakens the sandbox and never
falls back to concurrent work in the shared checkout.

### Integration branches

The default `--integration-branch=auto` creates one Taskflow integration ref in
each affected repository. Task PRs target those refs, and after all selected
tasks land, one final PR per repository targets the immutable `base_branch`.
Taskflow verifies each provisioned worktree's actual base before dispatch; a
successful provisioning command alone is not evidence. Because `never` must
leave task PRs open, `--merge=never` is accepted only with
`--integration-branch=off`. A submodule-only run also records the superproject
as a landing repository so its gitlink update and final PR cannot be skipped.
Taskflow archives only after both all task rows and all final landing rows are
merged and verified.
...
set, so a plain `$taskflow:taskflow-execute <slug>` loads one file and no more.
